...
describe('Basic Email Tags Input', () => {
  ...
  describe('user typing different email edge cases', () => {
    ...
    for (const testEmail of supportedEmailEdgeCases) {
      it(`should be allowed to add ${testEmail} as an email tag`, async () => {
        await addEmailTag(page, testEmail)
        await assertEmailTagCount(page, 1)
      })
    }

    it('spaces provided inside email are omitted while creating email tag', async () => {
      await addEmailTag(page, 'em a il@s pa ce. co')
      await assertInputText(page, 'email@space.co')
    })
  })
